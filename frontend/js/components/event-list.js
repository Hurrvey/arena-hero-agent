...
export function renderEvents(events) {
  ...
  return `<ul class="event-list">${[...events].reverse().slice(0, 40).map((event) => {
    ...
    return `<li class="event-row"><span class="mono muted">${timeLabel(event.at)}</span><span class="event-mark ${danger}">${danger === "danger" ? "!" : danger === "warn" ? "△" : "✓"}</span><span>${escapeHtml(type)}</span><span class="mono muted">Tick ${escapeHtml(event.tick ?? "—")}</span></li>`;
  }).join("")}</ul>`;
}
